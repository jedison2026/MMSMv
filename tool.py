...
def get_task_name(path):
    task_name = get_header(path)[1:]   #[1:] 表示从这个列表中提取从第二个元素开始的所有元素，并将这些元素赋值给 task_name
    
    return task_name

def load_data(path,args):         #根据文件路径加载数据，包含文件路径和参数对象
    with open(path) as file:
        reader = csv.reader(file)
        next(reader)             #跳过 CSV 文件的第一行，这通常是标题行
        lines = []
        for line in reader:
            lines.append(line)
        data = []               #初始化一个空列表 data，用于存储处理后的数据对象
        for line in lines:      #再次遍历 lines 列表，对于每一行数据，创建一个 MoleData 对象（MoleData 函数未在代码中定义），并将这些对象添加到 data 列表中。
            one = MoleData(line,args)
            data.append(one)
        
        data = MoleDataSet(data)   #将 data 列表转换为 MoleDataSet 对象（MoleDataSet 函数未在代码中定义），这可能是一个封装了数据操作的类。

    
        fir_data_len = len(data)    #记录原始数据长度，并打印出有效的 SMILES（化学结构表示）数量
        data_val = []
        smi_exist = []     #将有效的smile的索引添加到 smi_exist 列表中
        for i in range(fir_data_len):
            if data[i].mol is not None:
                smi_exist.append(i)
        data_val = MoleDataSet([data[i] for i in smi_exist])     #使用列表推导式和 smi_exist 列表中的索引来创建一个新的 MoleDataSet 对象 data_val，它只包含有效的 SMILES 数据
        now_data_len = len(data_val)     #获取处理后的数据集长度
...
def split_data(data,type,size,seed,log):
    assert len(size) == 3             #使用断言确保 size 参数包含三个元素，且这三个元素的和为 1，这意味着分割比例是正确的
    assert sum(size) == 1
...
def get_label_scaler(data):        #标签缩放器
    smile = data.smile()
    label = data.label()
...
def load_model(path,cuda,log=None,pred_args=None):
    ...
    state_dict = state['state_dict']    #提取状态字典
